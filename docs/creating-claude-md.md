...
Claude will insert an **Environment** block and a **Coding Conventions** section above the schema reference. From that point on, every stored procedure or query Claude writes in that session will follow those rules automatically — no need to repeat them in each prompt.
